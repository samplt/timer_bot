...
TOKEN = '5191129355:AAGwcTNafDeCf71Oppkp1CVNCs4GhXpyz6s'
...
def pre_set_timer(query):
    ...
    bot.send_message(message.chat.id,
                     'Введите время для установленного таймера \n'
                     'Пример ввода: \n'
                     '1. 30 сек \n'
                     '2. 30 мин \n'
                     '3. 30 час')
...
def set_text(message, times):
    ...
    timedelta = datetime.timedelta(days=0, seconds=times['сек'], minutes=times['мин'], hours=times['час'])
    cur_date += timedelta
